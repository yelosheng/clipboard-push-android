...
void ProcessReceivedFile(const std::string &filePath,
...
void HandleIncomingAnnouncement(const nlohmann::json &data) {
  ...
  if (local_url.empty() || transfer_id.empty())
    return;

  LOG_INFO("Receiver Mode: Peer announced file via LAN. ID: %s",
           transfer_id.c_str());
...
void PerformCloudUpload(const std::vector<uint8_t> &encData,
...
void PushFileData(const std::vector<uint8_t> &data, const std::string &filename,
                  const std::string &fileType) {
  auto &config = Config::Instance().Data();
  if (config.room_key.empty())
    return;
  ...
  std::string transfer_id =
      "tr_" + std::to_string(ms) + "_" + std::to_string(rand() % 100);
  ...
  announce["local_url"] = "http://" + LocalServer::Instance().GetIP() + ":" +
                          std::to_string(LocalServer::Instance().GetPort()) +
                          "/files/" + filename;
  announce["sent_at_ms"] = ms;

  SocketIOService::Instance().Emit("file_available", announce);
  LOG_INFO("tx file_available: id=%s, room=%s", transfer_id.c_str(),
  ...
  std::thread([pending, localPath, transfer_id, timeoutSecs]() {
    ...
    for (int i = 0; i < timeoutSecs * 20; ++i) {
      if (pending->completed || pending->upload_requested ||
          pending->need_relay)
        break;
    ...
    if (pending->completed) {
      ...
      if (!pending->upload_started.exchange(true)) {
        LOG_INFO("upload start: id=%s (reason: %s)", transfer_id.c_str(),
                 pending->upload_requested
                     ? "server_directed"
                     : (pending->need_relay ? "app_fallback" : "timeout"));
...
void ShowNotification(const std::wstring &title, const std::wstring &message,
                      UI::NotificationStyle style) {
  auto &config = Config::Instance().Data();
  if (!config.show_notifications)
    return;

  auto *data = new UI::NotificationData{title, message, style};
  if (!PostMessageW(g_hMsgWnd, WM_SHOW_NOTIFICATION, 0,
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam,
  ...
  ClipboardPush::Platform::ClipboardMonitor::Instance().HandleMessage(
  ...
  case WM_COMMAND:
    ...
    case IDM_TRAY_PUSH: {
      ...
      if (cb.type == ClipboardPush::Platform::ClipboardType::Text &&
        ...
        ClipboardPush::UI::TrayIcon::Instance().ShowMessage(
            L"Clipboard Pushed", L"Text content sent successfully");
      } else if (cb.type == ClipboardPush::Platform::ClipboardType::Image) {
        ClipboardPush::PushImage(cb.image_data);
        ClipboardPush::UI::TrayIcon::Instance().ShowMessage(
            L"Clipboard Pushed", L"Image content sent successfully");
        ...
        ClipboardPush::UI::TrayIcon::Instance().ShowMessage(
            L"Clipboard Pushed", L"File(s) sent successfully");
      }
    } break;
    ...
    case IDC_SETTINGS_RECONNECT: {
      ...
      ClipboardPush::SocketIOService::Instance().Connect(
          d.relay_server_url, d.room_id, d.device_id);
    } break;
...
int APIENTRY wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  HANDLE hMutex = CreateMutexW(
      NULL, TRUE, L"Global\\ClipboardPushWin32_SingleInstance_Mutex");
  ...
  HWND hWnd = CreateWindowExW(0, CLASS_NAME, L"Clipboard Push v3.0", 0, 0, 0, 0,
  ...
  sio.SetCallbacks(
      [](const std::string &content, bool encrypted) {
        ...
        std::wstring wMsg =
            ClipboardPush::Utils::ToWide(finalText.substr(0, 30));
      ...
      [](const nlohmann::json &data) {
        ...
        std::thread([data]() { OnRemoteFileReceived(data); }).detach();
      },
      [](ConnectionStatus status) {
        ...
        HICON hNew = ClipboardPush::Platform::CreateStatusIcon(
            GetModuleHandle(NULL), IDI_APP_ICON, color);
        if (hNew) {
      ...
      [](int secondsLeft) {
        std::wstring statusStr =
            L"Retrying in " + std::to_wstring(secondsLeft) + L"s...";
        ClipboardPush::UI::MainWindow::Instance().SetStatus(statusStr);
      });
  sio.SetSignalingCallback(
      [](const std::string &event, const nlohmann::json &data) {
          ...
          if (data.contains("peers") && data["peers"].is_array()) {
            for (const auto &peer : data["peers"]) {
              ...
              if (!cid.empty() && cid != config.device_id) {
                // Prefer device_name for display, fallback to client_id
                std::string displayName = peer.value("device_name", cid);
                peerNames.push_back(displayName);
              }
            }
          }
        ...
        if (event == "peer_evicted") {
          ...
          SocketIOService::Instance().Connect(config.relay_server_url,
                                              config.room_id, config.device_id);
          return;
        }
        ...
        if (transfer_id.empty())
          transfer_id = data.value("file_id", "");
        if (transfer_id.empty())
          return;
        ...
        if (g_pendingPushes.count(transfer_id)) {
          ...
          if (event == "transfer_command") {
            ...
            LOG_INFO("rx transfer_command: action=%s, reason=%s, id=%s",
                     action.c_str(), reason.c_str(), transfer_id.c_str());
  ...
  ClipboardPush::Platform::ClipboardMonitor::Instance().SetCallback([]() {
    if (g_isProcessingRemoteSync || g_activePeerCount <= 0)
      return;

    auto &config = Config::Instance().Data();
    if (!config.auto_push_text && !config.auto_push_image &&
        !config.auto_push_file)
      return;
    ...
    // Actually, let's keep it simple: if the push button would be disabled,
    // skip. This is safe because SetCallback runs on main thread or we can
    // check peer count.
    ...
    if (cb.type == ClipboardPush::Platform::ClipboardType::Text &&
        config.auto_push_text && !cb.text.empty()) {
      LOG_INFO("Auto-pushing text...");
      ClipboardPush::PushText(cb.text);
      ClipboardPush::ShowNotification(
          L"Auto Pushed", L"Text content sent automatically",
          ClipboardPush::UI::NotificationStyle::Outbound);
    } else if (cb.type == ClipboardPush::Platform::ClipboardType::Image &&
               config.auto_push_image) {
      LOG_INFO("Auto-pushing image...");
      ClipboardPush::PushImage(cb.image_data);
      ClipboardPush::ShowNotification(
          L"Auto Pushed", L"Image content sent automatically",
          ClipboardPush::UI::NotificationStyle::Outbound);
    } else if (cb.type == ClipboardPush::Platform::ClipboardType::Files &&
               config.auto_push_file) {
      ...
      ClipboardPush::ShowNotification(
          L"Auto Pushed", L"File(s) sent automatically",
          ClipboardPush::UI::NotificationStyle::Outbound);
    }
  });
  ...
  ClipboardPush::Platform::Hotkey::Instance().SetCallback([]() {
    if (g_activePeerCount <= 0) {
      ...
      ClipboardPush::ShowNotification(
          L"Push Failed", L"No target devices connected to this room.",
          UI::NotificationStyle::Inbound);
      return;
    }
    ...
    if (cb.type == ClipboardPush::Platform::ClipboardType::Text &&
      ...
      ClipboardPush::PushText(cb.text);
      ClipboardPush::ShowNotification(
          L"Clipboard Pushed", L"Text content sent",
          ClipboardPush::UI::NotificationStyle::Outbound);
    } else if (cb.type == ClipboardPush::Platform::ClipboardType::Image) {
      ClipboardPush::PushImage(cb.image_data);
      ClipboardPush::ShowNotification(
          L"Clipboard Pushed", L"Image content sent",
          ClipboardPush::UI::NotificationStyle::Outbound);
    } else if (cb.type == ClipboardPush::Platform::ClipboardType::Files) {
      for (const auto &file : cb.files) {
        ClipboardPush::PushPhysicalFile(file);
      ...
      ClipboardPush::ShowNotification(
          L"Clipboard Pushed", L"File(s) sent",
          ClipboardPush::UI::NotificationStyle::Outbound);
    }
  });
  ...
  if (!data.start_minimized) {
    ...
    ClipboardPush::ShowNotification(L"Clipboard Push v3.0",
                                    L"Running ultra-light in system tray!");
  }
